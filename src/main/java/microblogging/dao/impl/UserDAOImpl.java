...
public class UserDAOImpl implements UserDAO {
    ...
    // session.beginTransaction();
    //
    // User u = (User)session.createQuery("from User where username =
    // :name").setString("name", username).uniqueResult();
    // Set<User> currentFollowing = u.getFollowings();
    //
    // List<User> allUsersExceptMe = session.createQuery("from User where
    // username != :name").setString("name", username).list();
    ...
    //
    // session.update(a);
    // List<Blog> result = session.createQuery("from Blog where USER_ID =
    // :id").setLong("id", b.getId()).list();
